...
│   └── gateway/      # Express + Socket.IO gateway (also serves the web build)
├── services/
│   └── ai/           # FastAPI extraction / RAG / report service
├── packages/
│   └── shared/       # shared TypeScript contracts (socket events, API DTOs, domain)
├── seed/             # sample evidence for the "+ Add evidence" demo
├── docker-compose.yml
...
**Gateway:** `PORT`, `MONGO_URL`, `MONGO_DB`, `ES_URL`, `AI_SERVICE_URL`, `WEBHOOK_SECRET`, `DEGRADE_GRACEFULLY`, `CORS_ORIGIN`.
**AI service:** `OPENAI_API_KEY`, `OPENAI_MODEL` (default `gpt-4o-mini`), `PORT`.
...
```bash
# AI service — extractor unit tests (masking, org/person classification, graph build)
cd services/ai && .venv/bin/python -m pytest

# lint
.venv/bin/ruff check .                    # python
pnpm --filter @caselens/gateway typecheck # + web typecheck
```
...
Two always-on Docker services on Render (the gateway serves the web console same-origin, so no CORS and WebSockets work out of the box). Full walkthrough in **[DEPLOY.md](./DEPLOY.md)**.

---

## Demo script (≈3 min)

1. Open the deployed URL — the *Operation Meridian* case loads partially populated.
...
4. Both windows: the feed ticks, entities and edges **animate into the graph**, events drop onto the timeline.
5. **Brush** the timeline to a suspicious window → the graph filters to the entities active then.
6. **Ask the Case:** *"Summarize all contact between R. Mehta and the offshore account"* → the answer streams in with clickable evidence citations.
7. Click **Generate report** → a structured case report renders; export as markdown.

---

## Production next steps (deliberately out of scope)

Auth / RBAC (currently no access control), image-evidence OCR, a durable job queue (ingestion runs in-process), Kubernetes/GKE for scale-out, and tightened webhook rate-limiting. The graceful-degradation paths (in-memory fallbacks, LLM-optional) are demo conveniences — in production these become required managed services.
